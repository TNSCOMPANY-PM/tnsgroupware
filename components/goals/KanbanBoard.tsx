...
const COLUMNS = [
  { id: "todo",        label: "할 일",   dot: "bg-slate-400" },
  { id: "in_progress", label: "진행 중", dot: "bg-blue-500" },
  { id: "review",      label: "검토 중", dot: "bg-amber-500" },
  { id: "done",        label: "완료",    dot: "bg-emerald-500" },
];
...
function KanbanCardItem({
  card, onDelete, onEdit, onDragStart,
}: {
  card: KanbanCard;
...
export function KanbanBoard() {
  ...
  const addCard = async (col: string) => {
    ...
    const res = await fetch("/api/kanban", {
      ...
      body: JSON.stringify({ title, column: col, position: colCards.length, assignee: currentUserName }),
    });
  ...
  const saveEdit = async () => {
    ...
    const res = await fetch(`/api/kanban/${editCard.id}`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        title: editCard.title, description: editCard.description,
        priority: editCard.priority, assignee: editCard.assignee,
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {COLUMNS.map((col) => {
          const colCards = cards.filter((c) => c.column === col.id).sort((a, b) => a.position - b.position);
          return (
            <div
              key={col.id}
              className={cn(
              ...
              <div className="flex flex-col gap-2 px-3 pb-3 min-h-[4rem]">
                {colCards.map((card) => (
                  <KanbanCardItem
                    ...
                    onDelete={deleteCard} onEdit={setEditCard} onDragStart={setDragging}
                  />
                ))}
                {addingCol === col.id && (
                  <div className="rounded-xl border border-blue-200 bg-white p-3 shadow-sm">
                    ...
                    <div className="mt-2 flex gap-2">
                      <button type="button" onClick={() => addCard(col.id)} disabled={!newTitle.trim()}
                        className="rounded-lg bg-blue-600 px-3 py-1 text-xs font-semibold text-white hover:bg-blue-700 disabled:opacity-40">추가</button>
                      <button type="button" onClick={() => setAddingCol(null)}
                        className="rounded-lg px-3 py-1 text-xs font-medium text-slate-500 hover:bg-slate-100">취소</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-400 focus:outline-none"
                  value={editCard.title} onChange={(e) => setEditCard({ ...editCard, title: e.target.value })} />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-600">설명</label>
                <textarea rows={3} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-400 focus:outline-none resize-none"
                  value={editCard.description ?? ""} onChange={(e) => setEditCard({ ...editCard, description: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-slate-600">담당자</label>
                  <input className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-400 focus:outline-none"
                    value={editCard.assignee ?? ""} onChange={(e) => setEditCard({ ...editCard, assignee: e.target.value })} />
                </div>
                <div>
                  ...
                  <input type="date" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-400 focus:outline-none"
                    value={editCard.due_date ?? ""} onChange={(e) => setEditCard({ ...editCard, due_date: e.target.value })} />
                </div>
              </div>
              <div>
