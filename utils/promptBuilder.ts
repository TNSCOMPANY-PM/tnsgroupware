...
function buildDataBlock(data: BrandData, official?: OfficialData): string {
  ...
  lines.push("");
  lines.push(`[비교 규칙]`);
  lines.push(`- 공정위 정보공개서에 공시된 브랜드는 실명 사용 가능 (공개 데이터)`);
  lines.push(`- 단, 비방·비하·주관적 평가 금지. 수치 비교만 허용`);
  lines.push(`- 비교 수치는 반드시 출처 명시 (공정위 정보공개서 20XX년)`);
  lines.push(`- DATA 블록에 경쟁 브랜드 수치가 있을 때만 실명 비교`);
  lines.push(`- DATA에 없는 브랜드는 "업계 평균" 또는 "A브랜드"로 표기`);
...
1. answer-box: "${data.brand.name} vs 경쟁 브랜드" 형태의 비교 결론. 단독 소개 금지.
   예: "${data.brand.name} 실투자금 1,500만원. A브랜드 2,500만원 대비 40% 낮다."

2. 비교 테이블 필수 2개 이상:
   - 창업비용 항목별 비교 (가맹금/교육비/보증금/총비용)
   - 매출·수익 비교 (월매출/마진율/투자회수)
   [공정위 공시 데이터]에 경쟁 브랜드가 있으면 실명 사용. 없으면 "A브랜드" 표기.

3. stat-row로 핵심 비교 수치 3열 (실투자금/월매출/회수기간)

4. FAQ도 비교형: "창업비용이 가장 낮은 브랜드는?" 등
