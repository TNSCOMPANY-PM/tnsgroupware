...
import { Plus, Play, Trash2, ChevronRight, CheckCircle2, XCircle, TrendingUp, Bot, Search, MessageCircle, Download, Upload } from "lucide-react";
...
<!-- 답변 요약 -->
${(() => {
  const catMeta: Record<string, { intent: string; label: string }> = {
    "D0 개인창업 탐색": { intent: "창업에 관심 있는 일반인이 브랜드명 없이 질문. AI가 자발적으로 브랜드를 추천하는지 확인", label: "D0 — 개인창업 탐색" },
    "D1 프랜차이즈 탐색": { intent: "프랜차이즈 업종을 탐색하는 예비 창업자의 질문. 소자본/투자회수/초보자 키워드에서 브랜드가 추천되는지 확인", label: "D1 — 프랜차이즈 탐색" },
    "D2 김밥 카테고리": { intent: "김밥/분식 카테고리를 구체적으로 질문. 해당 카테고리에서 브랜드가 언급되는지 확인", label: "D2 — 김밥/분식 카테고리" },
    "D3 오공김밥 직접": { intent: "브랜드명을 직접 넣어 질문. AI가 정확한 데이터(창업비용, 매출, 가맹점 수 등)를 제공하는지 확인", label: "D3 — 브랜드 직접 질문 (정확도)" },
  };
  return ["D0 개인창업 탐색", "D1 프랜차이즈 탐색", "D2 김밥 카테고리", "D3 오공김밥 직접"].map(cat => {
    const meta = catMeta[cat];
    const catItems = items.filter(i => (i.category ?? "") === cat);
    if (catItems.length === 0) return "";
    const isD3 = cat.startsWith("D3");
    const mentioned = catItems.filter(i => i.mentioned).length;
    const catScore = isD3
      ? (catItems.length > 0 ? Math.round(catItems.reduce((s, i) => s + i.accuracy_score, 0) / catItems.length) : 0)
      : (catItems.length > 0 ? Math.round((mentioned / catItems.length) * 100) : 0);
    const summaries = catItems.map(i => {
      const short = i.ai_response.replace(/\n+/g, " ").slice(0, 120);
      return isD3
        ? `• ${i.prompt_text.slice(0, 30)} → 정확도 ${i.accuracy_score}%`
        : `• ${i.prompt_text.slice(0, 30)} → ${i.mentioned ? "✅ 브랜드 노출" : "❌ 브랜드 미노출"}`;
    }).join("<br>");
    const conclusion = isD3
      ? (catScore >= 50 ? `AI가 ${brandName}에 대해 비교적 정확한 정보를 제공하고 있습니다.` : `AI가 ${brandName}에 대해 부정확하거나 불완전한 정보를 제공합니다. 콘텐츠 보강이 필요합니다.`)
      : (mentioned > 0 ? `${catItems.length}개 질문 중 ${mentioned}개에서 ${brandName}이 언급됨. ${catItems.length - mentioned > 0 ? `${catItems.length - mentioned}개 질문에서는 아직 미노출.` : "전체 노출 달성."}` : `${catItems.length}개 질문 모두에서 ${brandName}이 언급되지 않음. GEO 최적화 필요.`);
    return `<div class="section" style="page-break-inside:avoid">
      <div class="section-title">${meta.label}</div>
      <div class="insight">
        <div class="insight-title">질문 의도</div>
        <div class="insight-text">${meta.intent}</div>
      </div>
      <div class="insight" style="border-color:${isD3 ? '#2563eb' : catScore > 0 ? '#10b981' : '#ef4444'}">
        <div class="insight-title">${isD3 ? `평균 정확도: ${catScore}%` : `노출률: ${catScore}% (${mentioned}/${catItems.length})`}</div>
        <div class="insight-text">${summaries}</div>
      </div>
      <div class="insight" style="border-color:#f59e0b">
        <div class="insight-title" style="color:#d97706">결론</div>
        <div class="insight-text">${conclusion}</div>
      </div>
    </div>`;
  }).join("");
})()}
    ...
    <div class="q-row">
      ...
      <div class="q-badge">${isD3
        ...
        : item.mentioned
          ...
          : `<span class="tag tag-no">브랜드 미노출</span>`}</div>
    </div>
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] p-0 overflow-hidden">
            ...
            <div className="px-5 py-3 border-b border-slate-200 bg-white sticky top-0 z-10">
              ...
              {(() => {
                ...
                const catInfo: { cat: string; label: string; intent: string }[] = [
                  { cat: "D0 개인창업 탐색", label: "D0", intent: "일반 창업 질문 → 브랜드 자발적 추천 여부" },
                  { cat: "D1 프랜차이즈 탐색", label: "D1", intent: "프랜차이즈 탐색 → 브랜드 추천 여부" },
                  { cat: "D2 김밥 카테고리", label: "D2", intent: "김밥/분식 카테고리 → 브랜드 노출 여부" },
                  { cat: "D3 오공김밥 직접", label: "D3", intent: "브랜드 직접 질문 → 답변 정확도" },
                ];
                return (
                        ...
                        <div key={cat} className={cn("border-l-4 rounded-r-lg px-3 py-2", isD3 ? "bg-blue-50 border-blue-300" : score > 0 ? "bg-emerald-50 border-emerald-400" : "bg-red-50 border-red-300")}>
                          ...
                          <p className="text-[11px] text-slate-500 mt-1">
                            {ci.map(i => isD3
                              ...
                              : `${i.prompt_text.slice(0, 22)}... ${i.mentioned ? "✅" : "❌"}`
                            ).join(" · ")}
                          </p>
                        </div>
                      );
