...
${monthRuns.map((r, ri) => {
  ...
  return `<div class="section">
    ...
    ${["D0 개인창업 탐색", "D1 프랜차이즈 탐색", "D2 김밥 카테고리", "D3 오공김밥 직접"].map(cat => {
      const catItems = items.filter((i: CheckItem) => (i.category ?? "") === cat);
      if (catItems.length === 0) return "";
      const isD3 = cat.startsWith("D3");
      return `<p style="font-size:11px;font-weight:600;color:#94a3b8;margin:12px 0 6px">${cat}</p>
      ${catItems.map((item: CheckItem, idx: number) => `<div class="qa">
        <div class="q">Q${idx + 1}. ${item.prompt_text}</div>
        <div class="a">${item.ai_response.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div>
        ${isD3 ? `<span class="tag tag-acc">정확도 ${item.accuracy_score}%</span>` : item.mentioned ? `<span class="tag tag-yes">브랜드 노출</span>` : `<span class="tag tag-no">브랜드 미노출</span>`}
      </div>`).join("")}`;
    }).join("")}
  </div>${ri < monthRuns.length - 1 ? '<div class="page-break"></div>' : ''}`;
}).join("")}
...
<div class="footer">Frandoor GEO Checker · ${month} 월간 리포트 · ${brandName}</div>
  ...
  const downloadReport = (run: CheckRun) => {
    ...
    // 같은 월 run들로 추이 그래프 데이터
    const month = run.run_date.slice(0, 7);
    const monthRuns = [...runs].filter(r => r.run_date.startsWith(month)).reverse();
    const trendExpData = monthRuns.map(r => getRunStats(r).expScore);
    const trendAccData = monthRuns.map(r => getRunStats(r).avgAcc);
    const trendLabels = monthRuns.map(r => r.run_date.slice(5));
...
${monthRuns.length > 1 ? `<div class="section">
  <div class="section-title">추이 그래프 (${month})</div>
