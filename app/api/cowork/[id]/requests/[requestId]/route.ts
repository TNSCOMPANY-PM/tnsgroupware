...
export async function PATCH(
  ...
  if (req.to_id !== String(session.employeeId)) return forbidden();
  ...
  await supabase.from("cowork_activities").insert({ cowork_id: id, actor_id: String(session.employeeId), actor_name: session.name, action: actionMap[body.status] ?? "request_updated", target_title: req.title });
