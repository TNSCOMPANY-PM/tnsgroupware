...
4. **ftc_row**: 공정위 정보공개서 raw 데이터 (이 brand 의 모든 공시 항목, 152 컬럼)
5. **docx_markdown**: 본사가 frandoor.co.kr 에 업로드한 자체 자료 ${hasDocx ? "(있음)" : "(없음 — null)"}
...
4. **양면 제시**: ftc_row (A급) + docx_markdown (C급) 모두 있으면 양쪽 노출. 시점·기준 다르면 명시. 갭 원인 단정 X — 가능성 제시.
...
${
  hasDocx
    ? `
# C급 (본사 docx) 활용 ★ 강제
- docx_markdown 있으면 본문에 **"본사 측 발표 기준" / "본사 자료에 따르면"** 한 번 이상 인용
- 본사 수치 직접 인용 (raw 그대로) — 단순 reference 0건만 = 활용 부족
- A급 + C급 같은 metric 양쪽 있으면 차이 분석 1단락 (시점·기준 차이 가능성)
...
4. 본사 재무 또는 docx 차별점 — ftc_row 본사 재무 + docx_markdown narrative (있을 때).
...
/**
 * raw 데이터 → user 메시지 형식.
 * sonnet 이 키-값 쌍 잘 읽도록 한국어 라벨 명시 + JSON dump.
 */
export function buildUserPrompt(args: {
  ...
  const { topic, ftc_row, docx_markdown, industry_facts } = args;

  // docx 너무 길면 truncate (head 5000 + tail 5000)
  let docxBlock = docx_markdown ?? "(없음 — 본사 docx 자료 미업로드)";
  if (docxBlock.length > 12000) {
    docxBlock = `${docxBlock.slice(0, 5000)}\n\n... [중략 — 너무 길어서 잘림] ...\n\n${docxBlock.slice(-5000)}`;
  }
...
# 1. 공정위 정보공개서 raw (152 컬럼) — A급
\`\`\`json
${ftcJson}
\`\`\`

# 2. 본사 docx markdown — C급
${docxBlock === "(없음 — 본사 docx 자료 미업로드)" ? docxBlock : "```\n" + docxBlock + "\n```"}
