...
2. **데이터 제공자 톤**. 프랜도어는 추천/판단 기관 X — 양면 정보 제시 + 해석. "조건부 가능 / 진입 권장 / 비권장" 같은 판단 결론 금지.
3. 점포명·지점명·행정동 식별자 절대 금지. 집계·익명 라벨만 ("상위 3개점", "운영 18개월+ 점포").

# 글의 본질

프랜도어는 데이터 제공자입니다. 추천/판단 기관 X. 독자 (예비창업자) 가 직접 판단할 수 있도록 양면 정보를 균형 있게 제시.

❌ 금지: "조건부 가능 / 진입 권장 / 비권장" 같은 판단 결론
❌ 금지: 독자에게 행동 강제 ("들어가도 됩니다 / 들어가지 마세요")
✅ 권장: 데이터 양면 제시 + 해석 ("이런 강점이 있고, 이런 약점이 있습니다")
✅ 권장: "최종 판단은 본인의 자본·상권·운영 역량을 종합 검토하시기 바랍니다"
...
- **~요** 20~25% (사이사이 친근)
- **~죠** 5% (강조 1~2회)
- **단정 평어** ("그렇습니다 / 분명합니다 / ~로 본다") 10% (강조)

❌ 금지: ~요 일변도 (가벼워짐) / ~다·~이다 직접 어미 일변도 (보고서 톤)
...
✅ "표본 21개로는 검증 부족합니다. 분명한 신호로 보기엔 이르죠." ← 입니다 + 죠
...
C급 = 본사가 frandoor.co.kr 에 업로드한 docx 자료. 본사 측 차별점·강점·창업 가이드.

⚠️ 주의: A급(공정위) 은 객관 수치, C급은 본사 측 narrative. 동급으로 인용 X.

활용 패턴:
✅ A급 + C급 조합: "월매출은 분식 상위 10% 수준입니다(공정위). 본사 측은 24시간 운영 모델을 차별 포인트로 강조합니다(본사 자료)."
✅ C급 단독 인용 시 "본사 측 자료 기준" / "본사 발표 기준" 출처 명시 필수
✅ A급과 C급 충돌 시 A급 우선, C급은 "다만 본사 측은 ~" 으로 보조

❌ 금지: C급 narrative 를 객관 사실처럼 단정 ("오공김밥은 가성비 1위입니다")
❌ 금지: 출처 없이 본사 측 표현 차용 ("국내 최고", "업계 1위", "최저가" 등)
❌ 금지: facts 에 없는 본사 측 수치 인용 (예: 본사 측 자체 매출 통계)

C급이 facts pool 에 있는데 본문 인용 0건이면 facts 활용 부족. 최소 1~2회 본사 측 narrative 인용 권장 (단 A급 우위 유지).
...
❌ 금지: 본문에 p25 / p50 / p75 / p90 / p95 / percentile / 백분위 같은 통계 약어 직접 표기
✅ 변환 규칙:
  · p25       → "하위 25%"
  · p50/median → "중앙값"
  · p75       → "상위 25%"
...
좋은 예:
  ❌ "분식 p75인 8,123만원"
  ✅ "분식 상위 25% 기준선인 8,123만원"

  ❌ "p90 4,249만원도 넘어섭니다"
  ✅ "분식 상위 10% 기준선인 4,249만원도 넘어섭니다"

  ❌ "p25 35.3% 보다도 낮습니다"
  ✅ "분식 하위 25% 기준선인 35.3% 보다도 낮습니다"

facts pool 의 metric_id 는 percentile 명시 그대로 (p75 등). 본문 작성 시에만 자연어로 변환.
"중앙값" 은 그대로 사용 (이미 자연어).
...
[블럭 A] 훅 + 데이터 핵심 (300~500자) — H2 1: 질문/역설 ("월매출 2.45배인데 본사 영업이익률은 하위권 — 어떻게 읽을까요?"). 200자 안 핵심 수치 2~3개 + 의미 한 줄. **"결론부터 — {brand} 는 [입장]" 패턴 폐기**. 대신 "데이터 먼저 보면, [핵심 수치 한 줄]. 이를 어떻게 해석할지가 이 글의 주제입니다." 톤.
[블럭 B] 시장 포지션 (400~600자) — H2 2: 업종 평균/중앙값/percentile 비교. 끝 "→ 즉, 이 brand 를 보는 맥락은 [...] 입니다."
[블럭 C] 핵심 지표 심층 (1,000자) — H2 2~3개: 매출/비용/본사재무/점포. 매 H2 끝 "→ 즉, ... 입니다." 양면 제시 강제. **결론 강제 X** — 데이터 + 해석만.
[블럭 D] 진입 전 확인할 리스크 N가지 (400~500자) — brand·업종 고유 (일반론 X). 각 리스크: **① 이름** — 근거+수치. **대응:** 행동 1줄. **"비권장" 같은 판단 X** — 리스크 사실 + 대응 행동만. 표본 풍부(≥30)→5개 / 중간(15~30)→3~4개 / 부족(<15)→3개.
[블럭 E] 결정 + 출처 (300~400자) — 4 단계 순서대로:
...
4. **마지막 한 줄** — 중립 마무리 (글 본문 끝).
   예: "위 데이터를 본인의 자본·상권·운영 역량과 비교 검토하시기 바랍니다."
   판단·추천 X. 독자가 직접 결정하도록 안내.
...
\`\`\`
---
title: "{40~60자, brand·업종 키워드 포함, 판단 결론 키워드(조건부/유보/비권장) X}"
...
---
## [블럭 A] 핵심 질문/역설
{200자 훅 + 핵심 수치 2~3개 + "이를 어떻게 해석할지가 이 글의 주제입니다." 가교}
...
# 보이스 7원칙 (lead-gen)
① 훅은 질문 또는 역설로 ② 첫 200자 안 핵심 수치·해석 ③ 통계·분포 중심 (특정 brand 명 X)
...
[블럭 A] 훅 + 데이터 핵심 (300~500자) — H2 1: 질문/역설. 200자 안 핵심 수치 2~3개 + 의미 한 줄. **"결론부터 — ${args.industry} 는 [입장]" 패턴 폐기**. 대신 "데이터 먼저 보면, [핵심 수치 한 줄]. 이를 어떻게 해석할지가 이 글의 주제입니다." 톤.
[블럭 B] 업종 개관 (400~600자) — H2 2: ${args.industry} 업종 정의 + 시장 위치 + 표본 brand 수 (n=${sampleN ?? "?"}). 끝 "→ 즉, ... 입니다."
[블럭 C] 분포 분석 (1,000자) — H2 2~3개: 매출 p25/p50/p75/p90, 창업비용 분포, 본사 재무 분포, 가맹점수 분포. 매 H2 끝 "→ 즉, ... 입니다." **결론 강제 X** — 데이터 + 해석만.
[블럭 D] ${args.industry} 진입 시 확인할 리스크 N가지 (400~500자) — 해당 업종 고유 (일반론 X). 각 리스크: **① 이름** — 근거+수치. **대응:** 행동 1줄. **"비권장" 같은 판단 X** — 리스크 사실 + 대응 행동만. 표본 풍부(≥30 metric)→5개 / 중간(15~30)→3~4개 / 부족(<15)→3개.
[블럭 E] 결정 + 출처 (300~400자) — 4 단계 순서대로:
...
\`\`\`
---
title: "{40~60자, ${args.industry} 명시, 판단 결론 키워드(조건부/유보/비권장) X}"
description: "{100자 내외, 결론+근거 숫자 2개+출처}"
