...
- stores_resolved: { count, source, as_of, note? }
    - source = "C_honsa_pos" : 본사 POS 있음. 최우선 인용.
    - source = "B_frandoor_docx" : docx 수작업 누적. "정보공개서 원문 {yr} 연말 누적" 형식.
    - source = "A_ftc" : 공정위 OpenAPI 2년차+ 누적. 병치 OK (갭 의미 1문장).
    - source = "A_ftc_first_year" : **최초 등록 해**. FTC frcsCnt 를 "가맹점수"로만 부르기 금지.
      반드시 "{yr}년 공정위 최초 등록 당시 {count}개로 출발" 식으로 시간축 해석.
    - source = "unknown" : 수치 미공개. 추정값 생성 금지.
...
## A-C 갭 해석 규칙 (stores_resolved.source 기반 분기)
source 값에 따라 본문 서술 방식을 달리한다:

- **"A_ftc_first_year"** (공정위 최초 등록 해):
  → "공정위엔 N개"로 단순 병치 절대 금지. FTC frcsCnt 는 해당 연도 등록 기준치이지 현 운영수가 아님.
  → "{yr}년 공정위 최초 등록 당시 {N}개로 출발 → {C.as_of} 본사 POS 기준 {M}개로 약 {배수}배 확장" 식으로 **시간축 해석**.
  → 독자 판단 포인트: 첫 등록 후 2~3년간 확장 속도.

- **"A_ftc"** (2년차+ 공정위 누적):
  → "공정위 {yr} 기준 {N}개 vs 본사 POS {as_of} 기준 {M}개" 병치 OK.
  → 갭의 의미 (최신성 차이, 공정위 갱신 주기 등) 1문장 박기.

- **"B_frandoor_docx"** (docx 수작업 누적):
  → "정보공개서 원문 {yr} 연말 누적 {N}개" 명시. OpenAPI 출처와 혼용 금지.

- **"C_honsa_pos"** (본사 POS 있음):
  → "본사 POS {as_of} 기준 활성 {N}개" 최우선 인용. A/B 수치는 참고로만.
...
- FTC frcsCnt 를 "가맹점수"로만 부르기 (반드시 "공정위 {yr} 기준" 수식어 또는 "최초 등록 당시" 맥락).
- stores_resolved.source = "A_ftc_first_year" 인데 제목·첫 H2 에 "공정위 N개" 를 숫자만 박기.
