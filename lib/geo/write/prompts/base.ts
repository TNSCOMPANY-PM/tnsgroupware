...
UNIT DISAMBIGUATION (필수):
- 매출 수치는 반드시 "가맹점당 월 평균" 또는 "가맹점당 연 평균" 중 어떤 것인지 claim 에 명시.
  예: "월평균매출" → 가맹점당 월 평균 매출 (만원 단위)
      "연평균매출" → 가맹점당 연 평균 매출 (만원 단위)
- 본사 전체 매출 / 업종 합계 매출은 facts 에 넣지 말 것. 반드시 **가맹점 1개당** 값만.
- 단위는 만원·개·곳·%·개월 중 하나. "원" 단위 금지 (반드시 만원으로 변환).
- 공정위 정보공개서 연매출 → 월매출 변환 시 반드시 ÷12 적용하고 claim 에 "월평균매출 (연매출 ÷ 12)" 로 표기.
- 외식 프랜차이즈 가맹점당 월평균매출 상식 범위: 500만원 ~ 50,000만원 (5천만원). 이 범위 밖이면 단위 재확인 필수.
- 범위 가드: 월평균매출 500 미만은 연매출을 잘못 라벨링한 오염 가능성, 50,000 초과는 업종 합계 오염 가능성 — 재검증 후 재조회.
